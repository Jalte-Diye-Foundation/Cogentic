...
CONFIG = {
    ...
    # --- Explanation zone (right side) ---
    # Wider right-side box for explanation text.
    "expl_left_ratio":    0.42,
    # Right edge is computed from quote left margin for symmetric side margins.
    "expl_top_ratio":     0.23,
...
def generate_images():
    ...
    for i, (quote, explanation) in enumerate(data):
        ...
        # --- EXPLANATION ZONE (right side) ---
        expl_left    = int(W * CONFIG["expl_left_ratio"])
        expl_right   = W - margin_left
        expl_max_w   = max(1, expl_right - expl_left)
        ...
        # --- DRAW EXPLANATION (right-aligned, vertically centred in right-side zone) ---
        ...
        for line in expl_lines:
            ...
            line_x = max(expl_left, expl_right - line_w)
            draw_spaced_text(
                draw,
                (line_x, y),
                line,
